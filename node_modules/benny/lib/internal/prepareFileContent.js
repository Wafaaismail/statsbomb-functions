...
const prepareCSV = (summary, options) => {
    ...
    return json2csv_1.parse(results);
};
...
const prepareColors = (length, opacity) => {
    return Array.from({ length }, (_, i) => `rgba(${colors[i % colors.length]}, ${opacity})`);
};
                ...
                datasets: [
                  {
                    ...
                    backgroundColor: ${JSON.stringify(prepareColors(values.length, 0.8))},
                    borderColor: ${JSON.stringify(prepareColors(values.length, 1))},
                    borderWidth: 1,
                  },
                ],
              },
...
const prepareFileContent = multimethod_1.multi((_, options) => options.format, multimethod_1.method('csv', prepareCSV), multimethod_1.method('table.html', prepareHTMLTable), multimethod_1.method('chart.html', prepareHTMLChart), multimethod_1.method(prepareJSON));
exports.default = prepareFileContent;
